...
import { createRecord, validatePhotos, enforceTimer, } from '../services/record.service'; // ✅ validateExercise 제거
export const registerRecord = async (req, res, next) => {
    try {
        const groupId = Number(req.params.groupId);
        const { nickname, password, exercise, description, seconds, distanceKm, photos, timerId, } = req.body ?? {};
        if (!groupId ||
            !nickname ||
            !password ||
            !exercise ||
