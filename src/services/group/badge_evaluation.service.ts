...
export async function evaluateAndAwardBadges(groupId: number, tx = prisma): Promise<void> {
  ...
  // 참여자 10명 달성 배지 확인
  if (group._count.participants >= 10 && !existingBadges.has(BadgeType.PARTICIPATION_10)) {
    badgesToAward.push(BadgeType.PARTICIPATION_10);
  }
  
  // 기록 100개 달성 배지 확인
  if (group._count.records >= 100 && !existingBadges.has(BadgeType.RECORD_100)) {
